																										...
																										SizedInBytes
{
	...
	{
		final ContiguousMemoryInterface lContiguousMemory = getPlaneContiguousMemory(pPlaneIndex);
		final Pointer<Byte> lBridJPointer = lContiguousMemory.getBridJPointer(Byte.class);
		return lBridJPointer;
	}

	public ContiguousMemoryInterface getPlaneContiguousMemory(int pPlaneIndex)
	{
		final ContiguousMemoryInterface lContiguousMemory = ((AbstractOffHeapAccess) getPlane(pPlaneIndex)).getContiguousMemory();
		return lContiguousMemory;
	}

	public FragmentedMemoryInterface getFragmentedMemory()
	{
		final FragmentedMemoryInterface lFragmentedMemory = new FragmentedMemory();
		for (int i = 0; i < numSlices(); i++)
