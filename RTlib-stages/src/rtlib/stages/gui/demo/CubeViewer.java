...
import rtlib.stages.gui.CubeScene;
import rtlib.stages.gui.SnapshotView;
import rtlib.stages.gui.View3D;
...
public class CubeViewer extends Application
{
	...
	public void start(Stage stage) {
		...
		final Scene scene = new Scene(rootGroup, startWidth, startHeight, true);
		...
		viewTimer = new AnimationTimer() {
			@Override public void handle(long now) {
				secondView.drawView(layeredPane.getWidth()/2, layeredPane.getHeight()/2);
				thirdView.drawView(layeredPane.getWidth()/2, layeredPane.getHeight()/2);
				fourthView.drawView(layeredPane.getWidth()/2, layeredPane.getHeight()/2);
			}
		};

		layeredPane.getChildren().addAll(background1Pane, subScene, secondViewPane, thirdViewPane, fourthViewPane);
		...
		RadialGradient blackRG = new RadialGradient(0, 0, 0.5, 0.5, 1, true, CycleMethod.NO_CYCLE, stopsRG);
		Background blackBG = new Background(new BackgroundFill(blackRG, null, null));
		...
		LinearGradient blueLG = new LinearGradient(0, 0, 0, 1, true, CycleMethod.NO_CYCLE, stopsLG);
		Background blueBG = new Background(new BackgroundFill(blueLG, null, null));

		Background greenBG = new Background(new BackgroundFill(Color.MEDIUMTURQUOISE, null, null));

		Background black = new Background(new BackgroundFill(Color.BLACK, null, null));
...
//		Scene scene = new Scene(layout, Color.CORNSILK);
	...
	private VBox createControls(CubeScene cubeScene) {
		...
		slider.valueProperty().bindBidirectional( cubeScene.getCubeCenterGroup().rotateProperty() );
		...
		final Slider xStageSlider = createSlider( cubeScene.getCubeCenterGroup().getTranslateX(), cubeScene.getCubeCenterGroup().getTranslateX() + CubeScene.VIEWPORT_SIZE - cubeBB.getMaxX() * 2.2, "X-axis stage control" );
		xStageSlider.valueProperty().bindBidirectional( cubeScene.getCubeCenterGroup().translateXProperty() );
		final Label xStageValue = new Label( Double.toString( xStageSlider.getValue() ) );
		xStageValue.textProperty().bind(xStageSlider.valueProperty().asString("%.0f"));
		...
		final Slider yStageSlider = createSlider(cubeScene.getCubeCenterGroup().getTranslateY(), cubeScene.getCubeCenterGroup().getTranslateY() + CubeScene.VIEWPORT_SIZE - cubeBB.getMaxY()* 2.2, "Y-axis stage control");
		yStageSlider.valueProperty().bindBidirectional( cubeScene.getCubeCenterGroup().translateYProperty() );
		final Label yStageValue = new Label( Double.toString( yStageSlider.getValue() ) );
		yStageValue.textProperty().bind(yStageSlider.valueProperty().asString("%.0f"));
		...
		final Slider zStageSlider = createSlider(cubeScene.getCubeCenterGroup().getTranslateZ(), cubeScene.getCubeCenterGroup().getTranslateZ() + CubeScene.VIEWPORT_SIZE - cubeBB.getMaxZ()* 2.2, "Z-axis stage control");
		zStageSlider.valueProperty().bindBidirectional( cubeScene.getCubeCenterGroup().translateZProperty() );

		final Label zStageValue = new Label( Double.toString( zStageSlider.getValue() ) );
		zStageValue.textProperty().bind(zStageSlider.valueProperty().asString("%.0f"));
...
//		VBox controls = new VBox(10, rotate, cull, wireframe);
		VBox controls = new VBox(10, rotate, rStage, xStage, yStage, zStage);
		controls.setPadding(new Insets(10));
		return controls;
	}

	private Slider createSlider(final double minValue, final double maxValue, final String helpText) {
