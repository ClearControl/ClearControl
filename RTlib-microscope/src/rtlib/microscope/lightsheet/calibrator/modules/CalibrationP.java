...
public class CalibrationP
{
	...
	{
		double lAverageIntensity = 0;
		int lCount = 0;
		for (int d = 0; d < mNumberOfDetectionArmDevices; d++)
		{
			Double lAverageIntensityForDetectionArm = calibrate(pLightSheetIndex,
																d,
																3);
			if (lAverageIntensityForDetectionArm == null)
				continue;
			lAverageIntensity += lAverageIntensityForDetectionArm;
			lCount++;
		}
		...
		lAverageIntensity = lAverageIntensity / lCount;

		System.out.format(	"Average image intensity for all detection arms: %g \n",
							lAverageIntensity);
