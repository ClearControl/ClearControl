 ...
 */
public class Copy {
    public static OffHeapPlanarImg<UnsignedShortType, ShortOffHeapAccess> copy(OffHeapPlanarImg<UnsignedShortType,
            ShortOffHeapAccess> pSourceImg, long[] pInitLocation, long[] pDimensions) {
        ...
        if (pDimensions[0] == 0 & pDimensions[1] == 0 & pDimensions[2] == 0) {
            throw new IllegalArgumentException("Can't copy a substack with all dimenstions set to zero.");
        }
        if (pInitLocation[0] + pDimensions[0] >= lSourceDimX || pInitLocation[1] + pDimensions[1] >= lSourceDimY ||
...
//        OffHeapPlanarImg<UnsignedShortType, ShortOffHeapAccess> imgOut = (OffHeapPlanarImg<UnsignedShortType, ShortOffHeapAccess>) new OffHeapPlanarImgFactory<>()
//                .createShortInstance(pDimensions, new Fraction());
        OffHeapPlanarImg<UnsignedShortType, ShortOffHeapAccess> imgOut = null;
        ContiguousMemoryInterface contOut = new OffHeapMemory("memmm", imgOut, OffHeapMemoryAccess.allocateMemory
                (2*pDimensions[0]*pDimensions[1]*pDimensions[2]), 2*pDimensions[0]*pDimensions[1]*pDimensions[2]);

        imgOut = (OffHeapPlanarImg<UnsignedShortType, ShortOffHeapAccess>) new OffHeapPlanarImgFactory()
                .createShortInstance(contOut, pDimensions, new UnsignedShortType());
        ...
        System.out.println("imgOut dims: " + imgOut.dimension(0) + " " + imgOut.dimension(1) + " " + imgOut
                .dimension(2));
        IterableInterval img = Views.offsetInterval(pSourceImg, pInitLocation, pDimensions);
...
//            System.out.println("init img: " + lCursor.get() + "pos: " + lCursor.getIntPosition(0) + " " + lCursor
