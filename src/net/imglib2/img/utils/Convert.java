...
public class Convert {
    public static void convertTIFFToRaw(String pPathToTIFF, String pPathToRaw) {
        try (FileOutputStream f = new FileOutputStream(pPathToRaw)) {
            Img<UnsignedShortType> img = (Img<UnsignedShortType>) new ImgOpener().openImgs(pPathToTIFF).get(0);
...
//                        System.out.println("integer is: " + curr);
                        arr[2 * (i + x * j + x * y * k)] = (byte) ((mask1 & (ra.get().getInteger())) >>> 8);
                        arr[2 * (i + x * j + x * y * k) + 1] = (byte) ((mask2 & ra.get().getInteger()));
//                        System.out.println("byte1 is: " +arr[2 * (i + x * j + x * y * k)]);
//                        System.out.println("byte2 is: " +arr[2 * (i + x * j + x * y * k) + 1]);
                    }
                }
            }
