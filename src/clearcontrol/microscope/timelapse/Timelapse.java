...
{
  private Variable<TimelapseTimerInterface> mTimelapseTimer =
                                                            new Variable<>("TimelapseTimer",
                                                                           null);

  private Variable<Boolean> mLimitNumberOfTimePointsVariable =
                                                             new Variable<>("LimitNumberOfTimePoints",
                                                                            true);

  private Variable<Boolean> mLimitTimelapseDurationVariable =
                                                            new Variable<>("LimitTimelapseDuration",
                                                                           false);

  private Variable<Boolean> mLimitTimelapseDateTimeVariable =
                                                            new Variable<>("LimitTimelapseDateTime",
                                                                           false);

  private Variable<Long> mMaxNumberOfTimePointsVariable =
                                                        new Variable<Long>("MaxNumberOfTimePoints",
                                                                           1L);

  private Variable<Long> mMaxDurationVariable =
                                              new Variable<Long>("MaxDuration",
                                                                 1L);

  private Variable<TimeUnit> mMaxDurationUnitVariable =
                                                      new Variable<TimeUnit>("MaxDurationUnit",
                                                                             TimeUnit.HOURS);

  private Variable<LocalDateTime> mMaxDateTimeVariable =
                                                       new Variable<LocalDateTime>("MaxDateTime",
                                                                                   LocalDateTime.now());

  private Variable<LocalDateTime> mStartDateTimeVariable =
                                                         new Variable<LocalDateTime>("StartDateTime",
                                                                                     LocalDateTime.now());

  private Variable<Long> mTimePointCounterVariable =
  ...
  public boolean loop()
  {
    ...
    if (mLimitTimelapseDurationVariable.get()
        && mMaxDurationVariable.get() != null)
      if (checkMaxDuration())
        return false;

    if (mLimitTimelapseDateTimeVariable.get()
        && mMaxDateTimeVariable.get() != null)
      if (checkMaxDateTime())
        return false;
  ...
  private boolean checkMaxDuration()
  {
    ...
    long lMaxDurationInNanos =
                             TimeUnit.NANOSECONDS.convert(mMaxDurationVariable.get(),
