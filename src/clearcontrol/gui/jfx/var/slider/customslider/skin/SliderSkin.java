                        ...
                        BehaviorSkinBase<Slider, SliderBehavior>
{
  ...
  private void initialize()
  {
    thumb = new StackPane()
    {
                                             ...
                                             Object... parameters)
      {
        ...
        default:
          return super.queryAccessibleAttribute(attribute,
                                                parameters);
        }
      }
    };
  ...
  StringConverter<Number> stringConverterWrapper =
                                                 new StringConverter<Number>()
                                                 {
                                                   ...
                                                   {
                                                     return slider.getLabelFormatter()
                                                                  .fromString(string);
                                                   }
                                                 };
                                ...
                                final double h)
  {
    ...
    if (getSkinnable().getOrientation() == Orientation.HORIZONTAL)
    {
      ...
      thumbTop =
               (int) (startY + ((trackAreaHeight - thumbHeight) / 2));

      positionThumb(false);
      // layout track
      track.resizeRelocate((int) (trackStart - trackRadius),
                           trackTop,
                           (int) (trackLength + trackRadius
                                  + trackRadius),
                           trackHeight);
      // layout tick line
      if (showTickMarks)
    ...
    else
    {
      ...
      double trackLeft =
                       (int) (startX
                              + ((trackAreaWidth - trackWidth) / 2));
      thumbLeft =
                (int) (startX + ((trackAreaWidth - thumbWidth) / 2));
                                   ...
                                   double leftInset)
  {
    ...
    {
      return (leftInset + minTrackLength()
              + thumb.minWidth(-1)
              + rightInset);
    }
    else
                                    ...
                                    double leftInset)
  {
    ...
    if (s.getOrientation() == Orientation.HORIZONTAL)
    {
      double axisHeight = showTickMarks ? (tickLine.prefHeight(-1)
                                           + trackToTickGap)
                                        : 0;
                                    ...
                                    double leftInset)
  {
    ...
    {
      double axisWidth = showTickMarks ? (tickLine.prefWidth(-1)
                                          + trackToTickGap)
                                       : 0;
      return leftInset
             + Math.max(thumb.prefWidth(-1), track.prefWidth(-1))
             + axisWidth
             + rightInset;
    }
  }
                                     ...
                                     double leftInset)
  {
    ...
    {
      return topInset
             + Math.max(thumb.prefHeight(-1), track.prefHeight(-1))
             + ((showTickMarks) ? (trackToTickGap
                                   + tickLine.prefHeight(-1))
                                : 0)
             + bottomInset;
    }
    else
