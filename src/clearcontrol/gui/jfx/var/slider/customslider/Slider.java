package clearcontrol.gui.jfx.var.slider.customslider;
 ...
 * <code>max</code>, and <code>value</code>. The <code>value</code> should
 * always be a number within the range defined by <code>min</code> and
...
public class Slider extends Control
{
	...
	/**
	 * The maximum value represented by this Slider. This must be a value greater
	 * than {@link #minProperty() min}.
	 */
	...
	/**
	 * The minimum value represented by this Slider. This must be a value less
	 * than {@link #maxProperty() max}.
	 */
	...
	/**
	 * The current value represented by this Slider. This value must always be
	 * between {@link #minProperty() min} and {@link #maxProperty() max},
	 * inclusive. If it is ever out of bounds either due to {@code min} or
	 * {@code max} changing or due to itself being changed, then it will be
	 * clamped to always remain valid.
	 */
	...
	public final BooleanProperty valueChangingProperty()
	{
		...
		{
			valueChanging = new SimpleBooleanProperty(this,
																								"valueChanging",
																								false);
		}
	...
	/**
	 * The orientation of the {@code Slider} can either be horizontal or vertical.
	 */
	...
	/**
	 * Specifies whether the {@link javafx.scene.control.Skin} implementation
	 * should show tick marks.
	 */
	 ...
	 * <p>
	 * This value should be positive and should be a value less than the span. Out
	 * of range values are essentially the same as disabling tick marks.
	 */
	...
	/**
	 * The number of minor ticks to place between any two major ticks. This number
	 * should be positive or zero. Out of range values will disable disable minor
	 * ticks, as will a value of zero.
	 */
	...
	/**
	 * Indicates whether the {@link #valueProperty() value} of the {@code Slider}
	 * should always be aligned with the tick marks. This is honored even if the
	 * tick marks are not shown.
	 */
	 ...
	 * {@link javafx.scene.control.Skin} implementation.
	 */
	...
	public final ObjectProperty<StringConverter<Double>> labelFormatterProperty()
	{
		...
		{
			labelFormatter = new SimpleObjectProperty<StringConverter<Double>>(	this,
																																					"labelFormatter");
		}
	 ...
	 * {@link #snapToTicksProperty() snapToTicks} is true then the nearest tick
	 * mark to the adjusted value will be used.
	 */
	 ...
	 * <code>value</code>is the actual amount between the {@link #minProperty()
	 * min} and {@link #maxProperty() max}. This function also takes into account
	...
	/**
	 * Increments the value by {@link #blockIncrementProperty() blockIncrement},
	 * bounded by max. If the max is less than or equal to the min, then this
	 * method does nothing.
	 */
	...
	/**
	 * Decrements the value by {@link #blockIncrementProperty() blockIncrement},
	 * bounded by max. If the max is less than or equal to the min, then this
	 * method does nothing.
	 */
	...
	/**
	 * Ensures that min is always < max, that value is always somewhere between
	 * the two, and that if snapToTicks is set then the value will always be set
	 * to align with a tick mark.
	 */
	...
	/**
	 * Utility function which, given the specified value, will position it either
	 * aligned with a tick, or simply clamp between min & max value, depending on
	...
	private double snapValueToTicks(double val)
	{
		...
		if (isSnapToTicks())
		{
			...
			{
				tickSpacing = getMajorTickUnit() / (Math.max(	getMinorTickCount(),
																											0) + 1);
			}
			else
			{
	...
	{
		private static final CssMetaData<Slider, Number> BLOCK_INCREMENT = new CssMetaData<Slider, Number>(	"-fx-block-increment",
		...
		private static final CssMetaData<Slider, Boolean> SHOW_TICK_LABELS = new CssMetaData<Slider, Boolean>("-fx-show-tick-labels",
		...
		private static final CssMetaData<Slider, Boolean> SHOW_TICK_MARKS = new CssMetaData<Slider, Boolean>(	"-fx-show-tick-marks",
																																																					BooleanConverter.getInstance(),
																																																					Boolean.FALSE)
		{
		...
		private static final CssMetaData<Slider, Boolean> SNAP_TO_TICKS = new CssMetaData<Slider, Boolean>(	"-fx-snap-to-ticks",
																																																				BooleanConverter.getInstance(),
																																																				Boolean.FALSE)
		{
		...
		private static final CssMetaData<Slider, Number> MAJOR_TICK_UNIT = new CssMetaData<Slider, Number>(	"-fx-major-tick-unit",
																																																				SizeConverter.getInstance(),
																																																				25.0)
		{
		...
		private static final CssMetaData<Slider, Number> MINOR_TICK_COUNT = new CssMetaData<Slider, Number>("-fx-minor-tick-count",
																																																				SizeConverter.getInstance(),
																																																				3.0)
		{
		...
		private static final CssMetaData<Slider, Orientation> ORIENTATION = new CssMetaData<Slider, Orientation>(	"-fx-orientation",
