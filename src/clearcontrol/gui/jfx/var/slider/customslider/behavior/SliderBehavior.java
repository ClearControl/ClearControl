...
public class SliderBehavior extends BehaviorBase<Slider>
{
	...
	public void trackPress(MouseEvent e, double position)
	{
		...
		{
			slider.adjustValue((1 - position) * (slider.getMax() - slider.getMin())
													+ slider.getMin());
		}
	}
	...
	public void thumbDragged(MouseEvent e, double position)
	{
		...
		slider.setValue(Utils.clamp(slider.getMin(),
																(position * (slider.getMax() - slider.getMin())) + slider.getMin(),
																slider.getMax()));
	}
	...
	private double snapValueToTicks(double val)
	{
		...
		{
			tickSpacing = slider.getMajorTickUnit() / (Math.max(slider.getMinorTickCount(),
																													0) + 1);
		}
		else
		{
