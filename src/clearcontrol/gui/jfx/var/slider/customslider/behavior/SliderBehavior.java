...
public class SliderBehavior extends BehaviorBase<Slider>
{
  ...
  protected static final List<KeyBinding> SLIDER_BINDINGS =
                                                          new ArrayList<KeyBinding>();
  static
  {
    SLIDER_BINDINGS.add(new KeyBinding(F4, "TraverseDebug").alt()
                                                           .ctrl()
                                                           .shift());

    SLIDER_BINDINGS.add(new SliderKeyBinding(LEFT, "DecrementValue"));
    SLIDER_BINDINGS.add(new SliderKeyBinding(KP_LEFT,
                                             "DecrementValue"));
    SLIDER_BINDINGS.add(new SliderKeyBinding(UP,
                                             "IncrementValue").vertical());
    SLIDER_BINDINGS.add(new SliderKeyBinding(KP_UP,
                                             "IncrementValue").vertical());
    SLIDER_BINDINGS.add(new SliderKeyBinding(RIGHT,
                                             "IncrementValue"));
    SLIDER_BINDINGS.add(new SliderKeyBinding(KP_RIGHT,
                                             "IncrementValue"));
    SLIDER_BINDINGS.add(new SliderKeyBinding(DOWN,
                                             "DecrementValue").vertical());
    SLIDER_BINDINGS.add(new SliderKeyBinding(KP_DOWN,
                                             "DecrementValue").vertical());
  ...
  protected/*final*/String matchActionForEvent(KeyEvent e)
  {
    ...
    if (action != null)
    {
      if (e.getCode() == LEFT || e.getCode() == KP_LEFT)
      {
        ...
        {
          action =
                 getControl().getOrientation() == Orientation.HORIZONTAL ? "IncrementValue"
                                                                         : "DecrementValue";
        }
      }
      else if (e.getCode() == RIGHT || e.getCode() == KP_RIGHT)
      {
        ...
        {
          action =
                 getControl().getOrientation() == Orientation.HORIZONTAL ? "DecrementValue"
                                                                         : "IncrementValue";
        }
      }
    }
  ...
  {
    final Slider slider = getControl();
    slider.setValue(Utils.clamp(slider.getMin(),
                                (position * (slider.getMax()
                                             - slider.getMin()))
                                                 + slider.getMin(),
                                slider.getMax()));
  }
  ...
  private double snapValueToTicks(double val)
  {
    ...
    if (slider.getMinorTickCount() != 0)
    {
      tickSpacing = slider.getMajorTickUnit()
                    / (Math.max(slider.getMinorTickCount(), 0) + 1);
    }
    else
    {
      tickSpacing = slider.getMajorTickUnit();
    ...
    double nextTickValue = (prevTick + 1) * tickSpacing
                           + slider.getMin();
  ...
  double computeIncrement()
  {
    ...
    if (slider.getBlockIncrement() > 0
        && slider.getBlockIncrement() < tickSpacing)
    {
      return tickSpacing;
    }
  ...
  public static class SliderKeyBinding extends OrientedKeyBinding
  {
    ...
    public SliderKeyBinding(KeyCode code,
