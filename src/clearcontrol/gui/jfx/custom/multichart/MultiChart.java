...
public class MultiChart extends HBox
{
	...
	public MultiChart(Class<?> pChartClass)
	{
		...
		mCheckListView.setOnMouseClicked((e) -> {
			...
			{
				MultiChartListItem lCurrentItemSelected = mCheckListView.getSelectionModel()
																																.getSelectedItem();
									...
									.addListener(new ListChangeListener<MultiChartListItem>()
									{
										...
										public void onChanged(ListChangeListener.Change<? extends MultiChartListItem> pChange)
										{
											...
											for (int i = 0; i < lNumberOfItems; i++)
											{
												...
												boolean lSelected = mCheckListView.getSelectionModel()
																													.isSelected(i);
		...
		mCheckListView.getCheckModel()
									...
									.addListener(new ListChangeListener<MultiChartListItem>()
									{
										...
										public void onChanged(ListChangeListener.Change<? extends MultiChartListItem> pChange)
										{
											...
											{
												boolean lChecked = mCheckListView.getCheckModel()
																													.isChecked(lItem);
		...
		if (pChartClass == LineChart.class)
			...
			throw new UnsupportedOperationException("Can't create chart for: " + pChartClass);
		...
		// mLineChart.setStyle(".default-color0.chart-symbol { -fx-background-color: #860061, white; -fx-background-insets: 0, 2; -fx-background-radius: 5px; -fx-padding: 5px;}");
	...
	public ObservableList<Data<Number, Number>> addSeries(String pSeriesLabel)
	{
		...
		MultiChartListItem lMultiChartListItem = new MultiChartListItem(pSeriesLabel,
																																		lSeries);
