 ...
 * 15:35
 */
public class RadialBargraphSkin extends SkinBase<RadialBargraph> implements
																																Skin<RadialBargraph>
{
	...
	public RadialBargraphSkin(RadialBargraph radialBargraph)
	{
		...
		angle = new SimpleDoubleProperty(	this,
																			"angle",
																			getSkinnable().getValue() * angleStep);
	...
	protected void handleControlPropertyChanged(final String PROPERTY)
	{
				...
				public void run()
				{
					...
					value.setText(String.format(Locale.US,
																			"%." + getSkinnable().getDecimals()
																					+ "f",
																			currentValue));
					value.setTranslateX((size - value.getLayoutBounds()
																						.getWidth()) * 0.5);
					bar.setLength(-currentValue * angleStep);
		...
		{
			thresholdRotate.setAngle(getSkinnable().getThreshold() * angleStep
																- 180
																- getSkinnable().getStartAngle());
		}
	...
	private void handleMouseEvent(final MouseEvent MOUSE_EVENT)
	{
		...
		if (getSkinnable().isInteractive() && SRC.equals(threshold))
		{
			...
			{
				Point2D point = getSkinnable().sceneToLocal(MOUSE_EVENT.getSceneX(),
	...
	private void handleTouchEvent(final TouchEvent TOUCH_EVENT)
	{
		...
		if (SRC.equals(threshold))
		{
			...
			{
				Point2D point = getSkinnable().sceneToLocal(TOUCH_EVENT.getTouchPoint()
																																.getSceneX(),
														...
														final Rotate ROTATE)
	{
		...
		double newValue = Double.compare(interactiveAngle, 180) <= 0 ? (interactiveAngle + 180.0
																																		+ getSkinnable().getStartAngle() - 360) / angleStep
																																: (interactiveAngle - 180.0
																																		+ getSkinnable().getStartAngle() - 360) / angleStep;
		if (Double.compare(newValue, getSkinnable().getMinValue()) >= 0 && Double.compare(newValue,
	...
	{
		FadeTransition fadeUnitOut = new FadeTransition(Duration.millis(425),
																										unit);
		fadeUnitOut.setFromValue(1.0);
		fadeUnitOut.setToValue(0.0);
		FadeTransition fadeValueOut = new FadeTransition(	Duration.millis(425),
																											value);
		...
		FadeTransition fadeUnitIn = new FadeTransition(	Duration.millis(425),
																										unit);
		fadeUnitIn.setFromValue(0.0);
		fadeUnitIn.setToValue(1.0);
		FadeTransition fadeValueIn = new FadeTransition(Duration.millis(425),
																										value);
		fadeValueIn.setFromValue(0.0);
		fadeValueIn.setToValue(1.0);
		ParallelTransition parallelIn = new ParallelTransition(	fadeUnitIn,
																														fadeValueIn);

		ParallelTransition parallelOut = new ParallelTransition(fadeUnitOut,
																														fadeValueOut);
		...
		SequentialTransition sequence = new SequentialTransition(	parallelOut,
																															pause,
																															parallelIn);
		sequence.play();
	}
		...
		if (withinSpeedLimit && getSkinnable().isAnimated())
		{
			...
			final KeyValue KEY_VALUE = new KeyValue(angle,
																							...
																							Interpolator.SPLINE(0.5,
																																	...
																																	0.4,
																																	1.0));
			final KeyFrame KEY_FRAME = new KeyFrame(Duration.millis(getSkinnable().getAnimationDuration()),
																							KEY_VALUE);
	...
	private void drawTickMarks(final GraphicsContext CTX)
	{
		...
		double tmpMinValue = smallRange	? getSkinnable().getMinValue() * 10
																		: getSkinnable().getMinValue();
		double tmpMaxValue = smallRange	? getSkinnable().getMaxValue() * 10
																		: getSkinnable().getMaxValue();
		double tmpAngleStep = smallRange ? angleStep / 10 : angleStep;
		for (double angle = 0, counter = tmpMinValue; Double.compare(	counter,
																																	tmpMaxValue) <= 0; angle -= tmpAngleStep, counter++)
		{
			sinValue = Math.sin(Math.toRadians(angle + startAngle));
			cosValue = Math.cos(Math.toRadians(angle + startAngle));

			Point2D innerPoint = new Point2D(	center.getX() + size
																						* 0.388
																						* sinValue,
																				center.getY() + size
																						* 0.388
																						* cosValue);
			Point2D outerPoint = new Point2D(	center.getX() + size
																						* 0.485
																						* sinValue,
																				center.getY() + size
																						* 0.485
	...
	private final void drawSections(final GraphicsContext CTX)
	{
		...
		IntStream.range(0, getSkinnable().getSections().size())
							...
							.forEachOrdered(i -> {
								...
								if (Double.compare(SECTION.getStart(), MAX_VALUE) <= 0 && Double.compare(	SECTION.getStop(),
																																													MIN_VALUE) >= 0)
								{
	...
	private final void drawMarkers()
	{
		for (Marker marker : getSkinnable().getMarkers().keySet())
		{
			...
			getSkinnable().getMarkers()
										.get(marker)
										.setAngle(marker.getValue() * angleStep
															- 180
															- getSkinnable().getStartAngle());
		}
	}
	...
	private void recalculateBarGradient()
	{
		...
		for (Stop stop : getSkinnable().getBarGradient())
		{
			if (Double.compare(stop.getOffset(), minFraction) == 0 || Double.compare(	stop.getOffset(),
																																								maxFraction) == 0)
				continue;
			stops.add(stop);
		}
		barGradient = new ConicalGradient(new Point2D(size * 0.5,
																									size * 0.5),
																			offset,
																			stops);
	}
	...
	private void resize()
	{
		...
		if (size > 0)
		{
			...
			minMeasuredValue.relocate((size - minMeasuredValue.getPrefWidth()) * 0.5,
																size * 0.11);
			minMeasuredValueRotate.setPivotX(minMeasuredValue.getPrefWidth() * 0.5);
			minMeasuredValueRotate.setPivotY(size * 0.39);
			minMeasuredValueRotate.setAngle(getSkinnable().getMinMeasuredValue() * angleStep
																			- 180
																			- getSkinnable().getStartAngle());

			maxMeasuredValue.setPrefSize(0.03 * size, 0.03 * size);
			maxMeasuredValue.relocate((size - maxMeasuredValue.getPrefWidth()) * 0.5,
																size * 0.11);
			maxMeasuredValueRotate.setPivotX(maxMeasuredValue.getPrefWidth() * 0.5);
			maxMeasuredValueRotate.setPivotY(size * 0.39);
			maxMeasuredValueRotate.setAngle(getSkinnable().getMaxMeasuredValue() * angleStep
																			- 180
																			- getSkinnable().getStartAngle());
