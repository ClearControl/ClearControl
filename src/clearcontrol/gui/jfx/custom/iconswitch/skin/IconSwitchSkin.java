                            ...
                            implements Skin<IconSwitch>
{
  ...
  private void init()
  {
    ...
    if (getSkinnable().getPrefWidth() != PREFERRED_WIDTH
        || getSkinnable().getPrefHeight() != PREFERRED_HEIGHT)
    {
      aspectRatio = getSkinnable().getPrefHeight()
  ...
  private void initGraphics()
  {
    Font.loadFont(getClass().getResourceAsStream("/eu/hansolo/enzo/fonts/opensans-semibold.ttf"),
                  (0.5 * PREFERRED_HEIGHT)); // "OpenSans"
    font = Font.font("Open Sans", 0.5 * PREFERRED_HEIGHT);

    background = new Region();
    background.getStyleClass().setAll("background");
    background.setStyle("-switch-color: "
                        + Util.colorToCss((Color) getSkinnable().getSwitchColor())
                        + ";");
    ...
    thumb.setStyle("-thumb-color: "
                   + Util.colorToCss((Color) getSkinnable().getThumbColor())
                   + ";");
    ...
    moveToDeselected = new TranslateTransition(Duration.millis(30),
                                               thumb);
    moveToSelected = new TranslateTransition(Duration.millis(30),
                                             thumb);
  ...
  private void registerListeners()
  {
    ...
    getSkinnable().heightProperty()
                  .addListener(observable -> handleControlPropertyChanged("RESIZE"));
    getSkinnable().switchColorProperty()
                  .addListener(observable -> handleControlPropertyChanged("SWITCH_COLOR"));
    getSkinnable().thumbColorProperty()
                  .addListener(observable -> handleControlPropertyChanged("THUMB_COLOR"));
    getSkinnable().symbolColorProperty()
                  .addListener(observable -> handleControlPropertyChanged("SYMBOL_COLOR"));
    getSkinnable().textProperty()
                  .addListener(observable -> handleControlPropertyChanged("TEXT"));
    getSkinnable().selectedProperty()
                  .addListener(observable -> handleControlPropertyChanged("SELECTED"));
    pane.setOnMouseClicked(mouseEvent -> {
      if (null == getSkinnable().getToggleGroup()
          || getSkinnable().getToggleGroup().getToggles().isEmpty())
      {
  ...
  private void resize()
  {
    ...
    if (width > 0 && height > 0)
    {
      ...
      symbol.setPrefSize(height * 0.59375
                         * getSkinnable().getSymbolType().WIDTH_FACTOR,
                         height * 0.59375
                                                                        * getSkinnable().getSymbolType().HEIGHT_FACTOR);
      symbol.relocate(height * 0.15
                      + (height * 0.59375 - height * 0.59375
                                            * getSkinnable().getSymbolType().WIDTH_FACTOR)
                        * 0.5,
                      height * 0.18
                               + (height * 0.59375 - height * 0.59375
      ...
      text.setFont(font);
      text.setVisible(!getSkinnable().getText().isEmpty()
                      && SymbolType.NONE == getSkinnable().getSymbolType());
      ...
      thumb.setTranslateX(getSkinnable().isSelected() ? height * 1.625
                                                      : height
                                                        * 0.875);
