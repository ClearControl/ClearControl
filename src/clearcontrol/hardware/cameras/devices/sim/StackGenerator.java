...
public class StackGenerator
{
	...
	{
		final long lWidth = max(1, lCameraDeviceBase.getStackWidthVariable().get());
		final long lHeight = max(1, lCameraDeviceBase.getStackHeightVariable().get());
		final long lDepth = max(1, lCameraDeviceBase.getStackDepthVariable().get());
		...
		final int lNumberOfImagesPerPlane =
				lCameraDeviceBase.getNumberOfImagesPerPlaneVariable()	.get()
																																					.intValue();
		...
		final StackInterface lStack = lCameraDeviceBase.getStackRecycler().getOrWait(1,
																											TimeUnit.SECONDS,
																											lStackRequest);
