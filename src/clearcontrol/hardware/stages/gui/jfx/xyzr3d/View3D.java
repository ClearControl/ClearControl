  ...
  protected EventHandler<MouseEvent> mouseHandler()
  {
    ...
    return new EventHandler<MouseEvent>()
    {
      ...
      public void handle(MouseEvent event)
      {
        ...
        {
          viewingTranslate.setX(viewingTranslate.getX()
                                + (startX - event.getSceneX()) / 100);
          viewingTranslate.setY(viewingTranslate.getY()
                                + (startY - event.getSceneY()) / 100);
        }
        else if (event.getButton() == MouseButton.MIDDLE)
        {
          viewingTranslate.setZ(viewingTranslate.getZ()
                                + (event.getSceneY() - startY) / 40);
        }
