package clearcontrol.hardware.stages.gui.jfx.xyzr3d.controls;
                                 ...
                                 BehaviorSkinBase<CircleIndicator, BehaviorBase<CircleIndicator>>
{
  ...
  private static final String DONE =
                                   ControlResources.getString("ProgressIndicator.doneString");
  /**
  ...
  private static final CssMetaData<CircleIndicator, Paint> PROGRESS_COLOR =
                                                                                                                  ...
                                                                                                                  null)
                                                                          {
                                                                            ...
                                                                            {
                                                                              final CircleIndicatorSkin skin =
                                                                                                             (CircleIndicatorSkin) n.getSkin();
  ...
  private static final CssMetaData<CircleIndicator, Boolean> SPIN_ENABLED =
                                                                          new CssMetaData<CircleIndicator, Boolean>("-fx-spin-enabled",
                                                                                                                    ...
                                                                                                                    Boolean.FALSE)
                                                                          {
                                                                            ...
                                                                            {
                                                                              final CircleIndicatorSkin skin =
                                                                                                             (CircleIndicatorSkin) node.getSkin();
                                                                              return skin.spinEnabled == null
                                                                                     || !skin.spinEnabled.isBound();
                                                                            }
  ...
  protected void initialize()
  {
    ...
    else
    {
      ...
      determinateIndicator =
                           new DeterminateIndicator(control,
                                                    this,
                                                    progressColor.get());
  ...
  private class DeterminateIndicator extends Region
  {
    ...
    private void setFillOverride(Paint fillOverride)
    {
      if (fillOverride instanceof Color)
      {
        ...
        progress.setStyle("-fx-background-color: rgba("
                          + ((int) (255 * c.getRed()))
                          + ","
                          + ((int) (255 * c.getGreen()))
                          + ","
    ...
    protected void layoutChildren()
    {
      ...
      */
      ...
      final double indicatorRadius =
                                   snapSize(Math.min(Math.min(progressRadius
                                                              - pLeft,
                                                              progressRadius
                                                                       - pRight),
                                                     Math.min(progressRadius
                                                              - pTop,
                                                              progressRadius
                                                                      - pBottom)));

      // find size of spare box that fits inside indicator radius
      double squareBoxHalfWidth = Math.ceil(Math.sqrt(
                                                      (indicatorRadius
                                                       * indicatorRadius)
                                                      / 2));
      double squareBoxHalfWidth2 =
                                 indicatorRadius * (Math.sqrt(2) / 2);
      ...
      if (control.getWidth() >= textWidth
          && control.getHeight() >= textHeight)
      {
        if (!text.isVisible())
    ...
    protected double computePrefWidth(double height)
    {
      ...
      final double indicatorMax =
                                snapSize(Math.max(Math.max(iLeft,
                                                           iRight),
                                                  Math.max(iTop,
                                                           iBottom)));
      final double pLeft = progress.snappedLeftInset();
      ...
      final double progressMax =
                               snapSize(Math.max(Math.max(pLeft,
                                                          pRight),
                                                 Math.max(pTop,
                                                          pBottom)));
  ...
  private final class IndeterminateSpinner extends Region
  {
    ...
    private void rebuildTimeline()
    {
      if (mSpinEnabled)
      {
        ...
        final ObservableList<KeyFrame> keyFrames =
                                                 FXCollections.<KeyFrame> observableArrayList();
          ...
          if (child instanceof Region)
          {
            ...
            if (region.getShape() != null)
            {
              ...
              region.getTransforms()
                    .setAll(new Scale(scale, scale, 0, 0));
            }
            else
            {
              region.autosize();
