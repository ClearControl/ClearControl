 ...
 */
public class BasicSampleSimulator implements SampleSimulatorInterface, LoggingInterface {
    ...
    public BasicSampleSimulator(String pPathToFile, long[] pDimensions, String pName) {
        mName = pName;
        mDimensions = Arrays.copyOf(pDimensions, pDimensions.length);
        LinkedList<OffHeapPlanarImg<UnsignedShortType, ShortOffHeapAccess>> lImgs = new LinkedList<>();
        short[] arr = loadUnsignedShortSampleSpaceFromDisk(pPathToFile, (int) pDimensions[0], (int) pDimensions[1],
                (int) pDimensions[2]);
        OffHeapMemory cm = new OffHeapMemory("mem", this, OffHeapMemoryAccess.allocateMemory(
                (2 * pDimensions[0] * pDimensions[1] * pDimensions[2])), 2 * pDimensions[0] * pDimensions[1] * pDimensions[2]);
        cm.copyFrom(arr);


        OffHeapPlanarImg<UnsignedShortType, ShortOffHeapAccess> imgOH = (OffHeapPlanarImg<UnsignedShortType,
                ShortOffHeapAccess>) new OffHeapPlanarImgFactory()
                .createShortInstance(cm, pDimensions, new UnsignedShortType());
    ...
    public void setCurrentPosition(long[] pCurrentPosition) {
        ...
        if (isLogging) {
            info("[Basic Sample Simulator] Setting current position to [" + mCurrentPosition[0] + ", " +
                    mCurrentPosition[1] + ", " + mCurrentPosition[2] + "].");
        }
    }
    ...
    public void setCurrentDimensions(long[] pCurrentDimensions) {
        this.mCurrentDimensions = Arrays.copyOf(pCurrentDimensions, pCurrentDimensions.length);
        if (isLogging) {
            info("[Basic Sample Simulator] Setting current dimensions to [" + mCurrentDimensions[0] + ", " +
                    mCurrentDimensions[1] + ", " + mCurrentDimensions[2] + "].");
        }
    }
    ...
    public String toString() {
        return mName + "-" + mDimensions[0] + "x" + mDimensions[1] + "x" + mDimensions[2];
    }

    public static void main(String[] args) {
        String lPath = "./config_files/testWD.raw";
        long[] dims = {1313, 992, 65};
        long l = 2 * dims[0] * dims[1] * dims[2];
        OffHeapMemory oh = OffHeapMemory.allocateBytes(l);

        try {
            File f = new File(lPath);
            FileMappedMemoryRegion fmmr = new FileMappedMemoryRegion(f, l, StandardOpenOption.READ);
            fmmr.map();
        } catch (Exception e) {
            e.printStackTrace();
        }
