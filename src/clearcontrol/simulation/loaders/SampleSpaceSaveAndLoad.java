...
public class SampleSpaceSaveAndLoad {
    public static void saveBlankSampleSpaceOnDisk(int pDimX, int pDimY, int pDimZ, String pPath) {
        System.out.println("Saving a sample space with dimensions: " + pDimX + "x" + pDimY + "x" + pDimZ + "...");
        ...
        try (FileOutputStream f = new FileOutputStream(pPath)) {
            ...
            System.out.println("--- Buffering: no. SampleSpace saved in: " + ((t2 - t1) / 1000000.) + " ms");
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
    ...
    public static void saveBufferedBlankSampleSpaceOnDisk(int pDimX, int pDimY, int pDimZ, String pPath) {
        System.out.println("Saving a sample space with dimensions: " + pDimX + "x" + pDimY + "x" + pDimZ + "...");
        try (FileOutputStream f = new FileOutputStream(pPath); BufferedOutputStream bos = new BufferedOutputStream(f)) {
            byte[] arr = new byte[pDimX * pDimY * pDimZ];
            long t1 = System.nanoTime();
            bos.write(arr, 0, pDimX * pDimY * pDimZ);
            long t2 = System.nanoTime();

            System.out.println("Buffering: yes. SampleSpace saved in: " + ((t2 - t1) / 1000000.) + " ms");
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
            System.out.println("Success: saved to " + pPath);
    ...
    public static byte[] loadSampleSpaceFromDisk(String pPath, int pDimX, int pDimY, int pDimZ) {
