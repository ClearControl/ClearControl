package clearcontrol.devices.cameras.devices.hamamatsu.utils;
 ...
 * @author royer
 */
public class DcamJToVideoFrameConverter extends VirtualDevice
                                        implements
                                        ...
                                        AsynchronousExecutorServiceAccess
{
  ...
  private HamStackCamera mOrcaFlash4StackCamera;
  ...
  private final SingleUpdateTargetObjectVariable<StackInterface> mStackReference =
                                                                                 new SingleUpdateTargetObjectVariable<StackInterface>("OffHeapPlanarStack");

  private final Variable<Long> mNumberOfImagesPerPlaneVariable =
                                                               new Variable<Long>("NumberOfPhases",
                                                                                  1L);

  /**
   * @param pOrcaFlash4StackCamera
   * @param pDcamFrameReference
   * @param pFlipX
   */
  public DcamJToVideoFrameConverter(final HamStackCamera pOrcaFlash4StackCamera,
                                    ...
                                    final boolean pFlipX)
  {
    ...
    mWaitForRecycledStackTimeInMicroSeconds =
                                            mOrcaFlash4StackCamera.getWaitForRecycledStackTimeInMicroSeconds();
    mMinimalNumberOfAvailableStacks =
                                    mOrcaFlash4StackCamera.getMinimalNumberOfAvailableStacks();
    mProcessingQueueSize =
                         mOrcaFlash4StackCamera.getStackProcessorQueueSize();
    ...
    final ProcessorInterface<Pair<TByteArrayList, DcamFrame>, StackInterface> lProcessor =
                                                                                         new ProcessorInterface<Pair<TByteArrayList, DcamFrame>, StackInterface>()
                                                                                         {

                                                                                           @Override
                                                                                           public StackInterface process(final Pair<TByteArrayList, DcamFrame> pInput)
                                                                                           {
                                                                                             // System.out.println("mAsynchronousConversionProcessor.process
                                                                                             // input.index=
                                                                                             // ->"
                                                                                             // +
                                                                                             // pInput.getIndex());
                                                                                             final StackInterface lOffHeapPlanarStack =
                                                                                                                                      convert(pInput);
    ...
    mAsynchronousConversionProcessor =
                                     new AsynchronousProcessor<Pair<TByteArrayList, DcamFrame>, StackInterface>("DcamJToVideoFrameConverter",
                                                                                                                mProcessingQueueSize,
                                                                                                                lProcessor);

    mSendToVariableAsynchronousProcessor =
                                         new AsynchronousProcessorBase<StackInterface, Object>("SendToVariableAsynchronousProcessor",
                                                                                               mProcessingQueueSize)
                                         {
                                           @Override
  ...
  protected StackInterface convert(final Pair<TByteArrayList, DcamFrame> pDcamFrame)
  {
    ...
    try
    {
      ...
      final long lNumberOfImagesPerPlane =
                                         mNumberOfImagesPerPlaneVariable.get();
      ...
      {
        RecyclerInterface<StackInterface, StackRequest> lStackRecycler =
                                                                       mOrcaFlash4StackCamera.getStackRecycler();

        final StackRequest lStackRequest =
                                         StackRequest.build(pDcamFrame.getRight()
                                                                      .getWidth(),
                                                            pDcamFrame.getRight()
                                                                      .getHeight(),
                                                            lNumberOfImagesKept);
        ...
        lOffHeapPlanarStack =
                            lStackRecycler.getOrWait(mWaitForRecycledStackTimeInMicroSeconds,
      ...
      ArrayList<Future> lFutureList =
                                    new ArrayList<Future>((int) lNumberOfImages);
      ...
      for (int i = 0, j = 0; i < lNumberOfImages; i++)
        if (pDcamFrame.getLeft().get(i) > 0)
        {
          final int fi = i;
          final int fj = j;
          final StackInterface lFinalOffHeapPlanarStack =
                                                        lOffHeapPlanarStack;

          Runnable lRunnable = () -> {
            final OffHeapMemory lTargetBuffer =
                                              (OffHeapMemory) lFinalOffHeapPlanarStack.getContiguousMemory(fj);

            if (mFlipX)
            {
              Pointer<Byte> lPointerForSinglePlane =
                                                   pDcamFrame.getRight()
              ...
              OffHeapMemory lSourceBuffer =
                                          OffHeapMemory.wrapPointer(lPointerForSinglePlane);
  ...
  public boolean open()
  {
    return mSendToVariableAsynchronousProcessor.start()
           && mAsynchronousConversionProcessor.start();
  }
  ...
  public boolean close()
  {
    return mAsynchronousConversionProcessor.stop()
           && mSendToVariableAsynchronousProcessor.stop();
  }
