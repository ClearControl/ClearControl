...
public class KdTree<T> extends KdNode<T>
{
  ...
  public MaxHeap<T> findNearestNeighbors(double[] searchPoint,
                                         ...
                                         DistanceFunction distanceFunction)
  {
    final BinaryHeap.Min<KdNode<T>> pendingPaths =
                                                 new BinaryHeap.Min<KdNode<T>>();
                                                      ...
                                                      double[] searchPoint)
  {
    ...
    while (!cursor.isLeaf())
    {
      ...
      final double otherDistance =
                                 distanceFunction.distanceToRect(searchPoint,
      ...
      if (evaluatedPoints.size() < desiredPoints
          || otherDistance <= evaluatedPoints.getMaxKey())
      {
    ...
    else
    {
      ...
      for (int i = 0; i < cursor.size(); i++)
      {
        ...
        final double distance =
                              distanceFunction.distance(point,
                                                        searchPoint);
