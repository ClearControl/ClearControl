									...
									final HashSet<String> pAllreadyIncludedFiles)
	{
		...
		while (lScanner.hasNext())
		{
			...
			if (lLine.startsWith("//include"))
			{
				...
				try
				{
					if (lIncludedFile.exists() && !pAllreadyIncludedFiles.contains(lFileName))
					{
			...
			else if (lLine.startsWith("//coreinclude"))
			{
				...
				if (!pAllreadyIncludedFiles.contains(lFileName))
				{
					...
					try
					{
						final InputStream lResourceAsStream = pClassForFindingScripts.getResourceAsStream(pPathForFindingScripts + lFileName);
