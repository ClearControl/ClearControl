...
public class CubeScene extends View3D
{
	...
	public void createBaseScene()
	{
		...
		Box stageBox = new Box( VIEWPORT_SIZE, VIEWPORT_SIZE, VIEWPORT_SIZE );

		stageBox.setMaterial( new PhongMaterial( Color.color( 0.1, 0.4, 0.4, 0.02 ) ) );
//		stageBox.setDrawMode( DrawMode.LINE );
		...
		final KeyValue kv0 = new KeyValue(cubeRotate.angleProperty(), 0, Interpolator.LINEAR);
		final KeyValue kv1 = new KeyValue(cubeRotate.angleProperty(), 360, Interpolator.LINEAR);
		...
		sceneDiameter = Math.sqrt(Math.pow( cubeBinL.getWidth(), 2) + Math.pow( cubeBinL.getHeight(), 2) + Math.pow( cubeBinL.getDepth(), 2));
	}
	...
	static void translateGroup( Group group, double offset )
	{
		...
		group.setTranslateX( -VIEWPORT_SIZE / 2.0 + (groupBB.getMinX() + groupBB.getMaxX()) / 2.0 + offset);
		group.setTranslateY( -VIEWPORT_SIZE / 2.0 + ( groupBB.getMinY() + groupBB.getMaxY() ) / 2.0 + offset );
		group.setTranslateZ( -VIEWPORT_SIZE / 2.0 + ( groupBB.getMinZ() + groupBB.getMaxZ() ) / 2.0 + offset );
	}
	...
	public void createSubScene(final double width, final double height, final SceneAntialiasing sceneAA)
	{
