...
public class Stage3DControl implements RunnableFX
{
	...
	private VBox createControls(CubeScene cubeScene) {
		...
		double xMax = cubeScene.getCubeCenterGroup().getTranslateX() + CubeScene.VIEWPORT_SIZE - cubeBB.getMaxX() * 2.2;

		final Slider xStageSlider = createSlider(0, 100, "X-axis stage control" );

		xStageSlider.valueProperty().addListener( new ChangeListener< Number >()
		{
			@Override public void changed( ObservableValue< ? extends Number > observable, Number oldValue, Number newValue )
			{
				cubeScene.getCubeCenterGroup().setTranslateX( newValue.doubleValue() *  (xMax - xMin) / 100 + xMin);
			}
		} );
//		xStageSlider.valueProperty().bindBidirectional( cubeScene.getCubeCenterGroup().translateXProperty() );
		...
		double yMax = cubeScene.getCubeCenterGroup().getTranslateY() + CubeScene.VIEWPORT_SIZE - cubeBB.getMaxY()* 2.2;
		final Slider yStageSlider = createSlider(0, 100, "Y-axis stage control");

		yStageSlider.valueProperty().addListener( new ChangeListener< Number >()
		{
			@Override public void changed( ObservableValue< ? extends Number > observable, Number oldValue, Number newValue )
			{
				cubeScene.getCubeCenterGroup().setTranslateY( newValue.doubleValue() * ( yMax - yMin ) / 100 + yMin );
			}
		} );
//		yStageSlider.valueProperty().bindBidirectional( cubeScene.getCubeCenterGroup().translateYProperty() );
		...
		double zMax = cubeScene.getCubeCenterGroup().getTranslateZ() + CubeScene.VIEWPORT_SIZE - cubeBB.getMaxZ()* 2.2;
		final Slider zStageSlider = createSlider(0, 100, "Z-axis stage control");

		zStageSlider.valueProperty().addListener( new ChangeListener< Number >()
		{
			@Override public void changed( ObservableValue< ? extends Number > observable, Number oldValue, Number newValue )
			{
				cubeScene.getCubeCenterGroup().setTranslateZ( newValue.doubleValue() * ( zMax - zMin ) / 100 + zMin );
			}
		} );
